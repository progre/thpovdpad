...
use windows::{
    ...
    Win32::{
        Devices::HumanInterfaceDevice::{
            ...
            DIJOYSTATE2, DISCL_BACKGROUND, DISCL_FOREGROUND, DISCL_NONEXCLUSIVE, DISCL_NOWINKEY,
        },
...
static mut ORIGINAL_I_DIRECT_INPUT_8_W_CREATE_DEVICE: usize = 0;
static mut ORIGINAL_I_DIRECT_INPUT_DEVICE_8_W_GET_DEVICE_STATE: usize = 0;

fn i_direct_input_device_8_get_device_state_hook(
    original_method_addr: usize,
    this: *const c_void,
    cb_data: u32,
    data: *mut c_void,
) -> HRESULT {
    type Func =
        extern "system" fn(this: *const c_void, cb_data: u32, lpv_data: *mut c_void) -> HRESULT;
    let func: Func = unsafe { transmute(original_method_addr) };
...
extern "system" fn i_direct_input_device_8_a_get_device_state_hook(
    ...
    i_direct_input_device_8_get_device_state_hook(
        unsafe { ORIGINAL_I_DIRECT_INPUT_DEVICE_8_A_GET_DEVICE_STATE },
...
extern "system" fn i_direct_input_device_8_w_get_device_state_hook(
    this: *const IDirectInputDevice8W,
    cb_data: u32,
    data: *mut c_void,
) -> HRESULT {
    i_direct_input_device_8_get_device_state_hook(
        unsafe { ORIGINAL_I_DIRECT_INPUT_DEVICE_8_W_GET_DEVICE_STATE },
        this as *const c_void,
        cb_data,
        data,
    )
...
extern "system" fn i_direct_input_8_w_create_device_hook(
    this: *const IDirectInput8W,
    guid: *const GUID,
    out_direct_input_device: *mut *mut IDirectInputDevice8W,
    unk_outer: *const IUnknown,
) -> HRESULT {
    type Func = extern "system" fn(
        this: *const IDirectInput8W,
    ...
    let func: Func = unsafe { transmute(ORIGINAL_I_DIRECT_INPUT_8_W_CREATE_DEVICE) };
    let result = func(this, guid, out_direct_input_device, unk_outer);

    if result.is_err() {
        return result;
    }
    if unsafe { ORIGINAL_I_DIRECT_INPUT_DEVICE_8_W_GET_DEVICE_STATE } == 0 {
        unsafe {
            let direct_input_device = *out_direct_input_device;
            let vtable = *(direct_input_device as *const _);
            setup_method_hook(
...
pub extern "system" fn DirectInput8Create(
    ...
    match unsafe { *riidltf } {
        ...
        IDirectInput8W::IID => {
            if unsafe { ORIGINAL_I_DIRECT_INPUT_8_W_CREATE_DEVICE } == 0 {
                unsafe {
                    let direct_input = *out;
                    let vtable = *(direct_input as *const _);
                    setup_method_hook(
